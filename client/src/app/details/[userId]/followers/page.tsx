...
import { getFollowers, User } from "@entities/user";
...
const FollowersPage = () => {
  ...
  const loadUsers = useCallback(
    async (isNext = false) => {
      ...
      setIsLoading(true);
      try {
        const result = await getFollowers(userId, isNext ? page : 1, pageSize);
        if (!isNext) {
          setUsers(result);
        } else {
  ...
  return (
    <SeparatorLayout>
      <PageHeader title="Followers" onBack={() => popView()} />
      <div className={classes.container}>
        ...
        {!hasMore && !isLoading && (
          <NoResultsFound
            label={
              users.length > 0 ? "No more followers." : "No followers yet."
            }
          />
        )}
      </div>
